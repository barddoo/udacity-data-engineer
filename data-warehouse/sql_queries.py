...
staging_events_copy = ("""
COPY staging_events FROM {} iam_role {} FORMAT AS json {};
""").format(config['S3']['LOG_DATA'], config['IAM_ROLE']['ARN'], config['S3']['LOG_JSONPATH'])

staging_songs_copy = ("""
COPY staging_songs FROM {} iam_role {} FORMAT AS json 'auto';
...
SELECT DISTINCT TIMESTAMP 'epoch' + (st_ev.ts / 1000) * INTERVAL '1 second' as start_time,
            st_ev.userId,
            st_ev.level,
            st_sg.song_id,
            st_sg.artist_id,
...
ON (st_sg.title = st_ev.song AND st_ev.artist = st_sg.artist_name)
